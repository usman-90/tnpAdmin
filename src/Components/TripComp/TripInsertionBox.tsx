...
const TripInsertionBox: React.FC<InsertionBoxProps> = ({ BoxState, BoxStateChange, fetchData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/getPackagesName`);
  ...
  const handleMenuClick = (e: any) => {
    const selectedPackage = data.find(item => item.package_id === Number(e.key));
    if (selectedPackage) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/getTrips`, {
  ...
  return (
    <Modal title="Add Trip" centered visible={BoxState} width={700} onOk={handleSubmit} onCancel={() => BoxStateChange(false)}>
