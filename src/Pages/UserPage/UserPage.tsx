...
const UserPage: React.FC = () => {
  ...
  const [deleteUserId, setDeleteUserId] = useState<number | null>(null); // State to store the id of the user to be deleted
  ...
  const deleteUser = async (id: number) => {
    ...
    await axios
      ...
      .then((_) => {
        ...
        fetchData();
        // Close the modal after user deletion
        closeDeleteModal();
      })
      .catch((err) => {
        Toast.fire({
  ...
  return (
    <>
      <div className="relative z-10 h-0 w-full">
        {(loading || deleting) && (
          <Loader message={loading ? "Fetching Data" : "Deleting Data"} />
        )}
      </div>
